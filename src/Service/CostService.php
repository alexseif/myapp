...
class CostService
{
    ...
    public function __construct(
      ...
      CurrencyService $currencyService
        ...
        $this->em = $em;
        $this->cost = $em->getRepository(CostOfLife::class)->sumCostOfLife(
        )['cost'];
    ...
    public function calc(): void
    {
        foreach ($this->currencies as $currency) {
            foreach ($this->units as $unit => $factor) {
                $raw = $this->cost * $factor['factor'] * ($currency->getEgp(
                    ) / 100);
                $round = (round($raw, $factor['precision'])) ?: round(
                  $raw,
                  $factor['precision'] + 1
                );
                $this->col[$unit][$currency->getCode()] = ($round) ?: $raw;
            }
        }
    }
